...
# --- Per-engine capture tool (record + review), when the engine has one ---
CAPTURE_SRC="$REPO_ROOT/engines/${ENGINE}_capture.py"
if [ -f "$CAPTURE_SRC" ]; then
    mkdir -p "$TARGET/tools"
    cp "$CAPTURE_SRC" "$TARGET/tools/capture.py"
    chmod +x "$TARGET/tools/capture.py"
    echo "Created tools/capture.py"
fi
